...
export const paymentService = {
  async createPendingPayment(
    payment: Omit<PendingPayment, 'created_at'>
  ...
  async getPendingPayment(reference: string): Promise<PendingPayment | null> {
    ...
    if (error && error.code !== 'PGRST116') {
      ...
      console.error(
        `Error fetching pending payment with reference ${reference}:`,
        error
      );
      throw error;
    }
  ...
  async deletePendingPayment(reference: string): Promise<void> {
    ...
    if (error) {
      console.error(
        `Error deleting pending payment with reference ${reference}:`,
        error
      );
      throw error;
    }
...
export const userTokenService = {
  ...
  async updateUserBalance(username: string, amount: number): Promise<void> {
    ...
    if (existingUser) {
      ...
      if (error) {
        console.error(
          `Error creating user token record for ${username}:`,
          error
        );
        throw error;
      }
